...
/// // socket.
/// let sender_socket = IcmpSocket::bind(&"127.0.0.1:0".parse()?)?;
/// let echoer_socket = IcmpSocket::bind(&"127.0.0.1:0".parse()?)?;
///
...
impl IcmpSocket {
    ...
    /// let socket = match IcmpSocket::bind(&"127.0.0.1:0".parse()?) {
    ...
    /// # }
    /// ```
    pub fn bind(addr: &SocketAddr) -> io::Result<IcmpSocket> {
        let socket = sys::unix::net_icmp::IcmpSocket::bind(addr)?;
        IcmpSocket::from_socket(socket)
    }
    ...
    pub fn from_socket(socket: sys::unix::net_icmp::IcmpSocket) -> io::Result<IcmpSocket> {
        Ok(IcmpSocket {
            sys: sys::IcmpSocket::new(socket)?,
    ...
    /// let socket = IcmpSocket::bind(&addr)?;
    /// #    Ok(())
    /// # }
    /// #
    /// # fn main() {
    ...
    ///
    /// The returned `IcmpSocket` is a reference to the same socket that this
    ...
    ///
    /// // We must bind it to an open address.
    /// let socket = IcmpSocket::bind(&"127.0.0.1:0".parse()?)?;
    ...
    /// use mio::net::IcmpSocket;
    ///
    /// let broadcast_socket = IcmpSocket::bind(&"127.0.0.1:0".parse()?)?;
    ...
    /// let socket = IcmpSocket::bind(&"127.0.0.1:0".parse()?)?;
    ///
    /// assert_eq!(socket.set_nonblocking(1)?, 255);
    /// #
    /// #    Ok(())
    /// # }
    /// #
